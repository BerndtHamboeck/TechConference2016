...
--select as admin, look at the rows
...
--ssn & CreditCardNumber encrypted?
--import certificate and connection options >>
--column encryption setting=enabled
SELECT * FROM Sales.CustomerPII
go

--hmmm, check the territories
select distinct TerritoryID 
from Sales.CustomerPII
order by TerritoryID
go
...
UPDATE Sales.CustomerPII SET FirstName = 'Changed' 
WHERE TerritoryID = 9 -- 0 rows affected
go

-- Blocked from inserting a new customer in a territory not assigned to him...
INSERT INTO Sales.CustomerPII 
...
--Programmability ->Table valued function: [Security].[customerAccessPredicate]
-- Security -> Security Policies: [Security].[customerPolicy]
